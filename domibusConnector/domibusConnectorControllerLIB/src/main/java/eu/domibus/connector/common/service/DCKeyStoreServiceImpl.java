 ...
 */
@Service
public class DCKeyStoreServiceImpl implements DCKeyStoreService {

    private static final Logger LOGGER = LogManager.getLogger(DCKeyStoreServiceImpl.class);

    private final ApplicationContext ctx;

    public DCKeyStoreServiceImpl(ApplicationContext ctx) {
        this.ctx = ctx;
    }
...
//        validatePathReadable();
    ...
    public void validateKeyExists(StoreConfigurationProperties storeConfigurationProperties, String alias, String password) {
        ...
        try {
            ...
            if (key == null) {
                throw new DCKeyStoreServiceImpl.ValidationException(String.format("No key found for alias [%s]"));
            }
        } catch (KeyStoreException e) {
            throw new DCKeyStoreServiceImpl.ValidationException(String.format("Key Store exception when retrieving key alias [%s]", alias), e);
        } catch (NoSuchAlgorithmException e) {
            throw new DCKeyStoreServiceImpl.ValidationException(String.format("No such key exception when retrieving key alias [%s]", alias), e);
        } catch (UnrecoverableKeyException e) {
            throw new DCKeyStoreServiceImpl.ValidationException(String.format("Validation exception when retrieving key alias [%s]", alias), e);
        }
    }
    ...
    public void validateCertExists(StoreConfigurationProperties storeConfigurationProperties, String alias) {
        ...
        try {
            ...
            if (certificate == null) {
                throw new DCKeyStoreServiceImpl.ValidationException(String.format("No certificate found for alias [%s]", alias));
            }
        } catch (KeyStoreException e) {
            throw new DCKeyStoreServiceImpl.ValidationException(String.format("Key store exception occured while loading certificate with alias [%s] from key store", alias), e);
        }
    }
